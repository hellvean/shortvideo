<template>
  <div class="tabs">
    ...
    <div class="page">
      <Page :total="data.totals" show-elevator show-total show-sizer :page-size-opts="sizes" :page-size="limit"
    ...
    <Modal
      ...
      <div class="table-task" v-bind="addUser">
        ...
        <div class="task-cz">
          ...
          <Input prefix="ios-star" placeholder="密码" type="password" v-model="addUser.password" style="width: auto"/>
        </div>
        <div class="task-cz">
          <span class="task-cz-name">
            昵称：
        ...
        <div class="task-cz">
          ...
          <Input placeholder="手机号"  v-model="addUser.mobile" style="width: auto"/>
        </div>
        <div class="task-cz">
          <span class="task-cz-name">
            角色：
          </span>
          <Select v-model="addUser.id" filterable style="width:200px" @on-change="selectChange">
    ...
    <Modal
      ...
      <div class="table-task">
        <div class="task-cz">
          ...
          <Input placeholder="用户名" prefix="ios-star" v-model="editname" style="width: auto"/>
        </div>
        <div class="task-cz">
          <span class="task-cz-name">
            手机号：
          </span>
          <Input placeholder="手机号" v-model="editmobile" style="width: auto"/>
        </div>
        <div class="task-cz">
          <span class="task-cz-name">
            角色：
          </span>
          <Select v-model="editroleId" filterable style="width:200px">
            <Option v-for="item in rolelist" :value="item.id" :key="item.id">{{ item.roleName}}</Option>
          </Select>
          <!--<Input prefix="ios-star" placeholder="角色" style="width: auto"/>-->
        </div>
  ...
  export default {
    ...
    methods: {
      editok: function () {
        ...
        this.$http({
          method: 'put',
          url: this.url + '/v1/user/updateUser/' + this.editId,
          data: {
      ...
      edit: function (i) {
        // console.log(i)
        this.editId = i.userId
        let _this = this;
        this.$http({
          method: 'get',
          url: this.url + '/v1/user/findUserById/' + this.editId,
          data: {},
          success: function (res) {
            // console.log(res)
            if (res.data.code == 20000) {
              _this.editname = res.data.data.name
              _this.editmobile = res.data.data.mobile;
              _this.editroleId = res.data.data.roleId;
